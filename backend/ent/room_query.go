...
func (rq *RoomQuery) QueryUser() *UserQuery {
...
// QueryRoom2problem chains the current query on the room2problem edge.
func (rq *RoomQuery) QueryRoom2problem() *ProblemQuery {
	query := &ProblemQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) WithUser(opts ...func(*UserQuery)) *RoomQuery {
	query := &UserQuery{config: rq.config}
...
//  WithRoom2problem tells the query-builder to eager-loads the nodes that are connected to
// the "room2problem" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoom2problem(opts ...func(*ProblemQuery)) *RoomQuery {
	query := &ProblemQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoom2problem = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withRoom2problem; query != nil {
		...
		query.Where(predicate.Problem(func(s *sql.Selector) {
			s.Where(sql.InValues(room.Room2problemColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Room2problem = append(node.Edges.Room2problem, n)
		}
	}
