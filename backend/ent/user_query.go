...
type UserQuery struct {
	...
	// eager-loading edges.
	withUser2room    *RoomQuery
	withUser2problem *ProblemQuery
...
// QueryUser2room chains the current query on the user2room edge.
func (uq *UserQuery) QueryUser2room() *RoomQuery {
...
// QueryUser2problem chains the current query on the user2problem edge.
func (uq *UserQuery) QueryUser2problem() *ProblemQuery {
...
//  WithUser2room tells the query-builder to eager-loads the nodes that are connected to
// the "user2room" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUser2room(opts ...func(*RoomQuery)) *UserQuery {
	query := &RoomQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUser2room = query
	return uq
}

//  WithUser2problem tells the query-builder to eager-loads the nodes that are connected to
// the "user2problem" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUser2problem(opts ...func(*ProblemQuery)) *UserQuery {
	query := &ProblemQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUser2problem = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withUser2room != nil,
			uq.withUser2problem != nil,
		}
	)
	...
	if query := uq.withUser2room; query != nil {
		...
		query.Where(predicate.Room(func(s *sql.Selector) {
			s.Where(sql.InValues(user.User2roomColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.User2room = append(node.Edges.User2room, n)
		}
	}

	if query := uq.withUser2problem; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Problem(func(s *sql.Selector) {
			s.Where(sql.InValues(user.User2problemColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.User2problem = append(node.Edges.User2problem, n)
		}
	}
